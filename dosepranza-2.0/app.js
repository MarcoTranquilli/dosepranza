import { initializeApp } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-app.js";
        ...
        function renderMenu() {
            ...
            document.getElementById('menu-container').innerHTML = filtered.map(i => {
                ...
                const disabled = !orderOpen ? 'opacity-50 pointer-events-none' : '';
                html += `<div class="card p-5 rounded-[2rem] flex flex-col justify-between ${disabled}">
                    ...
                    <div class="mt-4 flex justify-between items-center gap-2">${pricing}<button data-action="add-std" data-id="${i.id}" class="bg-primary text-white h-10 w-10 rounded-2xl shadow-lg flex items-center justify-center flex-shrink-0 active:scale-90" ${!orderOpen ? 'disabled' : ''}><i class="fas fa-plus"></i></button></div>
                </div>`;
                return html;
            }).join('');
        }
