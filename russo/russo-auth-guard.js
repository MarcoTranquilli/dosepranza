(() => {
  ...
  async function resetLocalSession() {
    ...
    if (window.caches?.keys) {
      tasks.push(window.caches.keys().then((keys) => Promise.all(keys.map((key) => window.caches.delete(key)))).catch(() => {}));
    }
    if (navigator.serviceWorker?.getRegistrations) {
      tasks.push(navigator.serviceWorker.getRegistrations().then((registrations) => Promise.all(registrations.map((registration) => registration.unregister()))).catch(() => {}));
    }
    await Promise.allSettled(tasks);
  }

  if (resetRequested) {
    resetLocalSession().finally(() => {
      window.location.replace('../?session=clean&v=' + Date.now());
    });
    return;
  }
