...
if(reset){clean().finally(()=>location.replace('../?session=clean&v=google-final-2'));return}
...
async function verify(){const access=window.DoseSupplierAccess;if(!access||access.isFilePreview?.()||access.isE2E?.()||ok())return;try{const s=await access.resolveSession?.();if(s||ok())return}catch(e){}location.replace('../?next=russo&v=google-final-2')}
verify();
})();