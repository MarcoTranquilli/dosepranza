...
if(reset){clean().finally(()=>location.replace('./?session=clean&v=russo-root-3'));return}
...
async function verify(){const access=window.DoseSupplierAccess;if(!access||access.isFilePreview?.()||access.isE2E?.()||ok())return;try{const s=await access.resolveSession?.();if(s||ok())return}catch(e){}location.replace('./?next=russo&v=russo-root-3')}
verify();
})();